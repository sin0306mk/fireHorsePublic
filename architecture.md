...
| スロット要素数 | `req[2]〜req[7]` の各テキスト | 非空スロットを全角スペースで分割した要素数が6未満 | `スロット[N]の要素数が想定と異なります` |

空スロット（`strip()` 結果が空文字）はスロット要素数チェックをスキップする。
また、以下のマーカー文字列を含むスロットは、火災通知ではなくサイト側の案内文であるためスロット要素数チェックをスキップする。

| マーカー | 文字列 | 用途 |
|---|---|---|
| 災害なし案内 | `災害は発生しておりません` | 「現在、災害は発生していない」旨の案内メッセージ |
| 複数同時出場補足 | `その他にも、市内で災害が発生しており` | 複数災害の同時出場時にサイトが付加する補足メッセージ |
...
**6 フィールド形式（スペース省略）の対応**

サイト側の入力ゆれで `[5]` と `[6]` が全角スペースなしで結合される場合がある（例: `付近で発生した火災は鎮火しました`）。
この場合、`[5]` の先頭にある「付近で発生した」または「付近より」プレフィックスを除去した残りを `status` として扱う。

**通知振り分け（status の前方一致）**

`status` は `startswith` で判定する（後続文字列が追加されても対応できるよう前方一致）。
`鎮火` 判定では `火災は鎮火` 形式（2026-05-13 実ページで確認）も対象とする。
